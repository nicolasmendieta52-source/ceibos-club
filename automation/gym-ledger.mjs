...
export function campaignFromLedger(summary, rows, sponsors = []) {
  ...
  if (!headers.every((h,i)=>rows?.[0]?.[i]===h) || rows.length !== COUNT+1) throw new Error('La planilla debe tener 100 ladrillos y las columnas esperadas');
  ...
  const addSponsor = name => {
    ...
    if (trimmed.toUpperCase() === 'EUROPCAR') return;
